// Code generated by protoc-gen-gogo. DO NOT EDIT.
// source: desc_test_wellknowntypes.proto

package testprotos

import (
	fmt "fmt"
	proto "github.com/gogo/protobuf/proto"
	types "github.com/gogo/protobuf/types"
	math "math"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.GoGoProtoPackageIsVersion3 // please upgrade the proto package

type TestWellKnownTypes struct {
	StartTime            *types.Timestamp   `protobuf:"bytes,1,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	Elapsed              *types.Duration    `protobuf:"bytes,2,opt,name=elapsed,proto3" json:"elapsed,omitempty"`
	Dbl                  *types.DoubleValue `protobuf:"bytes,3,opt,name=dbl,proto3" json:"dbl,omitempty"`
	Flt                  *types.FloatValue  `protobuf:"bytes,4,opt,name=flt,proto3" json:"flt,omitempty"`
	Bl                   *types.BoolValue   `protobuf:"bytes,5,opt,name=bl,proto3" json:"bl,omitempty"`
	I32                  *types.Int32Value  `protobuf:"bytes,6,opt,name=i32,proto3" json:"i32,omitempty"`
	I64                  *types.Int64Value  `protobuf:"bytes,7,opt,name=i64,proto3" json:"i64,omitempty"`
	U32                  *types.UInt32Value `protobuf:"bytes,8,opt,name=u32,proto3" json:"u32,omitempty"`
	U64                  *types.UInt64Value `protobuf:"bytes,9,opt,name=u64,proto3" json:"u64,omitempty"`
	Str                  *types.StringValue `protobuf:"bytes,10,opt,name=str,proto3" json:"str,omitempty"`
	Byt                  *types.BytesValue  `protobuf:"bytes,11,opt,name=byt,proto3" json:"byt,omitempty"`
	Json                 []*types.Value     `protobuf:"bytes,12,rep,name=json,proto3" json:"json,omitempty"`
	Extras               []*types.Any       `protobuf:"bytes,13,rep,name=extras,proto3" json:"extras,omitempty"`
	XXX_NoUnkeyedLiteral struct{}           `json:"-"`
	XXX_unrecognized     []byte             `json:"-"`
	XXX_sizecache        int32              `json:"-"`
}

func (m *TestWellKnownTypes) Reset()         { *m = TestWellKnownTypes{} }
func (m *TestWellKnownTypes) String() string { return proto.CompactTextString(m) }
func (*TestWellKnownTypes) ProtoMessage()    {}
func (*TestWellKnownTypes) Descriptor() ([]byte, []int) {
	return fileDescriptor_f1e62925231fcffe, []int{0}
}
func (m *TestWellKnownTypes) XXX_Unmarshal(b []byte) error {
	return xxx_messageInfo_TestWellKnownTypes.Unmarshal(m, b)
}
func (m *TestWellKnownTypes) XXX_Marshal(b []byte, deterministic bool) ([]byte, error) {
	return xxx_messageInfo_TestWellKnownTypes.Marshal(b, m, deterministic)
}
func (m *TestWellKnownTypes) XXX_Merge(src proto.Message) {
	xxx_messageInfo_TestWellKnownTypes.Merge(m, src)
}
func (m *TestWellKnownTypes) XXX_Size() int {
	return xxx_messageInfo_TestWellKnownTypes.Size(m)
}
func (m *TestWellKnownTypes) XXX_DiscardUnknown() {
	xxx_messageInfo_TestWellKnownTypes.DiscardUnknown(m)
}

var xxx_messageInfo_TestWellKnownTypes proto.InternalMessageInfo

func (m *TestWellKnownTypes) GetStartTime() *types.Timestamp {
	if m != nil {
		return m.StartTime
	}
	return nil
}

func (m *TestWellKnownTypes) GetElapsed() *types.Duration {
	if m != nil {
		return m.Elapsed
	}
	return nil
}

func (m *TestWellKnownTypes) GetDbl() *types.DoubleValue {
	if m != nil {
		return m.Dbl
	}
	return nil
}

func (m *TestWellKnownTypes) GetFlt() *types.FloatValue {
	if m != nil {
		return m.Flt
	}
	return nil
}

func (m *TestWellKnownTypes) GetBl() *types.BoolValue {
	if m != nil {
		return m.Bl
	}
	return nil
}

func (m *TestWellKnownTypes) GetI32() *types.Int32Value {
	if m != nil {
		return m.I32
	}
	return nil
}

func (m *TestWellKnownTypes) GetI64() *types.Int64Value {
	if m != nil {
		return m.I64
	}
	return nil
}

func (m *TestWellKnownTypes) GetU32() *types.UInt32Value {
	if m != nil {
		return m.U32
	}
	return nil
}

func (m *TestWellKnownTypes) GetU64() *types.UInt64Value {
	if m != nil {
		return m.U64
	}
	return nil
}

func (m *TestWellKnownTypes) GetStr() *types.StringValue {
	if m != nil {
		return m.Str
	}
	return nil
}

func (m *TestWellKnownTypes) GetByt() *types.BytesValue {
	if m != nil {
		return m.Byt
	}
	return nil
}

func (m *TestWellKnownTypes) GetJson() []*types.Value {
	if m != nil {
		return m.Json
	}
	return nil
}

func (m *TestWellKnownTypes) GetExtras() []*types.Any {
	if m != nil {
		return m.Extras
	}
	return nil
}

func init() {
	proto.RegisterType((*TestWellKnownTypes)(nil), "testprotos.TestWellKnownTypes")
}

func init() { proto.RegisterFile("desc_test_wellknowntypes.proto", fileDescriptor_f1e62925231fcffe) }

var fileDescriptor_f1e62925231fcffe = []byte{
	// 426 bytes of a gzipped FileDescriptorProto
	,
}
