...
func TestMessageRegistry_LookupTypes(t *testing.T) {
	...
	testutil.Ok(t, err)
	a := &types.Any{TypeUrl: "foo.bar/google.protobuf.DescriptorProto", Value: b}
	...
	dur := &types.Duration{Nanos: 100, Seconds: 1000}
	b, err = proto.Marshal(dur)
	testutil.Ok(t, err)
	a = &types.Any{TypeUrl: "foo.bar/google.protobuf.Duration", Value: b}
	pm, err = mr.UnmarshalAny(a)
	testutil.Ok(t, err)
	testutil.Ceq(t, dur, pm, eqm)
	testutil.Eq(t, reflect.TypeOf((*types.Duration)(nil)), reflect.TypeOf(pm))
		...
		Fields: []*types.Field{
			{
				...
				Cardinality: types.Field_CARDINALITY_OPTIONAL,
				JsonName:    "id",
			},
			{
				Name:        "name",
...
func createFetcher(t *testing.T) TypeFetcher {
	bol, err := types.MarshalAny(&types.BoolValue{Value: true})
	testutil.Ok(t, err)
	in32, err := types.MarshalAny(&types.Int32Value{Value: -42})
	testutil.Ok(t, err)
	uin32, err := types.MarshalAny(&types.UInt32Value{Value: 42})
	testutil.Ok(t, err)
	in64, err := types.MarshalAny(&types.Int64Value{Value: -420420420420})
	testutil.Ok(t, err)
	uin64, err := types.MarshalAny(&types.UInt64Value{Value: 420420420420})
	testutil.Ok(t, err)
	byt, err := types.MarshalAny(&types.BytesValue{Value: []byte{1, 2, 3, 4, 5, 6, 7, 8}})
	testutil.Ok(t, err)
	str1, err := types.MarshalAny(&types.StringValue{Value: "foo"})
	testutil.Ok(t, err)
	str2, err := types.MarshalAny(&types.StringValue{Value: "bar"})
	testutil.Ok(t, err)
	str3, err := types.MarshalAny(&types.StringValue{Value: "baz"})
	testutil.Ok(t, err)

	mapT := map[string]proto.Message{
		"https://foo.bar/some.Type": &types.Type{
			Name:   "some.Type",
			Oneofs: []string{"un"},
			Fields: []*types.Field{
				{
				...
				},
				{
					...
					JsonName:    "b",
					Number:      2,
					Cardinality: types.Field_CARDINALITY_REPEATED,
					Kind:        types.Field_TYPE_STRING,
				},
				{
					Name:        "c",
			...
			SourceContext: &types.SourceContext{FileName: "foo.proto"},
			Syntax:        types.Syntax_SYNTAX_PROTO3,
		},
		"https://foo.bar/some.OtherType": &types.Type{
			Name: "some.OtherType",
			Fields: []*types.Field{
				{
					Name:        "a",
			...
			SourceContext: &types.SourceContext{FileName: "bar.proto"},
			Syntax:        types.Syntax_SYNTAX_PROTO2,
		},
		"https://foo.bar/some.OtherType.AnotherType": &types.Type{
		...
		"https://foo.bar/some.Enum": &types.Enum{
			Name: "some.Enum",
		...
		"https://foo.bar/some.YetAnother.MessageType": &types.Type{
			...
			SourceContext: &types.SourceContext{FileName: "baz.proto"},
			Syntax:        types.Syntax_SYNTAX_PROTO2,
		},
	}
...
func getApi(t *testing.T) *types.Api {
	bol, err := types.MarshalAny(&types.BoolValue{Value: true})
	testutil.Ok(t, err)
	dbl, err := types.MarshalAny(&types.DoubleValue{Value: 10203040.506070809})
	testutil.Ok(t, err)
	flt1, err := types.MarshalAny(&types.FloatValue{Value: 3.14159})
	testutil.Ok(t, err)
	flt2, err := types.MarshalAny(&types.FloatValue{Value: 2.71828})
	testutil.Ok(t, err)
	enu, err := types.MarshalAny(&types.Int32Value{Value: int32(testprotos.ReallySimpleEnum_VALUE)})
	testutil.Ok(t, err)
	msg, err := types.MarshalAny(&testprotos.ReallySimpleMessage{Id: proto.Uint64(100), Name: proto.String("deuce")})
	testutil.Ok(t, err)
	return &types.Api{
		Name: "some.Service",
...
func TestMessageRegistry_MarshalAndUnmarshalAny(t *testing.T) {
	...
	// check that we can unmarshal it with normal typess library
	var umd descriptor.DescriptorProto
	err = types.UnmarshalAny(a, &umd)
	testutil.Ok(t, err)
...
func TestMessageRegistry_MessageDescriptorToPType(t *testing.T) {
	...
	testutil.Eq(t, types.Syntax_SYNTAX_PROTO2, msg.Syntax)
	...
	testutil.Eq(t, types.Field_CARDINALITY_OPTIONAL, msg.Fields[0].Cardinality)
	testutil.Eq(t, types.Field_TYPE_STRING, msg.Fields[0].Kind)
	...
	var v types.DynamicAny
	err = types.UnmarshalAny(msg.Fields[0].Options[0].Value, &v)
	testutil.Ok(t, err)
	testutil.Ceq(t, &types.BoolValue{Value: true}, v.Message, eqpm)

	testutil.Eq(t, "def", msg.Fields[1].Name)
	testutil.Eq(t, types.Field_CARDINALITY_REPEATED, msg.Fields[1].Cardinality)
	testutil.Eq(t, types.Field_TYPE_INT32, msg.Fields[1].Kind)
	...
	testutil.Eq(t, types.Field_CARDINALITY_OPTIONAL, msg.Fields[2].Cardinality)
	testutil.Eq(t, types.Field_TYPE_STRING, msg.Fields[2].Kind)
	...
	testutil.Eq(t, types.Field_CARDINALITY_OPTIONAL, msg.Fields[3].Cardinality)
	testutil.Eq(t, types.Field_TYPE_UINT64, msg.Fields[3].Kind)
	...
	testutil.Eq(t, types.Field_CARDINALITY_OPTIONAL, msg.Fields[4].Cardinality)
	testutil.Eq(t, types.Field_TYPE_STRING, msg.Fields[4].Kind)
...
func TestMessageRegistry_EnumDescriptorToPType(t *testing.T) {
	...
	testutil.Eq(t, types.Syntax_SYNTAX_PROTO2, enum.Syntax)
	...
	// make sure the value is a wrapped bool
	var v types.DynamicAny
	err = types.UnmarshalAny(enum.Options[0].Value, &v)
	testutil.Ok(t, err)
	testutil.Ceq(t, &types.BoolValue{Value: true}, v.Message, eqpm)
	...
	// make sure the value is a wrapped bool
	err = types.UnmarshalAny(enum.Enumvalue[1].Options[0].Value, &v)
	testutil.Ok(t, err)
	testutil.Ceq(t, &types.BoolValue{Value: true}, v.Message, eqpm)
