...
func (rc *resolutionContext) addType(url string, enum bool) error {
	...
	if enum {
		rc.recordEnum(url, m.(*types.Enum))
		return nil
	}
			...
			go func() {
				...
				if innerErr == nil {
					if d != nil {
						...
						innerErr = rc.addType(typeUrl, kind == types.Field_TYPE_ENUM)
					}
				}
...
func (rc *resolutionContext) recordEnum(url string, e *types.Enum) {
	rc.mu.Lock()
	defer rc.mu.Unlock()
...
func (rc *resolutionContext) recordType(url string, t *types.Type) (*fileEntry, string) {
	rc.mu.Lock()
	defer rc.mu.Unlock()
...
func (rc *resolutionContext) toFileDescriptors(mr *MessageRegistry) (map[string]*desc.FileDescriptor, error) {
	return toFileDescriptors(rc.files, func(tt *typeTrie, name string) (proto.Message, error) {
		mdp, edp := tt.typesToDescriptor(name, mr)
		if mdp != nil {
			return mdp, nil
		} else {
			return edp, nil
...
// typesToDescriptor converts this level of the trie into a message or enum
// descriptor proto, requiring that the element stored in t.typ is a *types.Type
// or *types.Enum. If t.typ is nil, a placeholder message (with no fields) is
...
// result in a panic.
func (t *typeTrie) typesToDescriptor(name string, mr *MessageRegistry) (*descriptor.DescriptorProto, *descriptor.EnumDescriptorProto) {
	...
	default:
		...
		if t.typ == nil {
			...
			msg = createMessageDescriptor(t.typ.(*types.Type), mr)
		}
		...
		for _, name := range keys {
			...
			chMsg, chEnum := nested.typesToDescriptor(name, mr)
			if chMsg != nil {
...
func createEnumDescriptor(e *types.Enum, mr *MessageRegistry) *descriptor.EnumDescriptorProto {
...
func createEnumValueDescriptor(v *types.EnumValue, mr *MessageRegistry) *descriptor.EnumValueDescriptorProto {
...
func createMessageDescriptor(m *types.Type, mr *MessageRegistry) *descriptor.DescriptorProto {
...
func createFieldDescriptor(f *types.Field, mr *MessageRegistry) *descriptor.FieldDescriptorProto {
...
func createServiceDescriptor(a *types.Api, mr *MessageRegistry) *descriptor.ServiceDescriptorProto {
...
func createMethodDescriptor(m *types.Method, mr *MessageRegistry) *descriptor.MethodDescriptorProto {
...
func createOptions(options []*types.Option, optionsDesc *desc.MessageDescriptor, mr *MessageRegistry) *dynamic.Message {
